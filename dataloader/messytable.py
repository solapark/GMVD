...
class Messytable(VisionDataset):
    def __init__(self, root, cam_set, train_cam, test_cam):
        ...
        self.worldgrid2worldcoord_mat = np.array([[0, .1, self.origin[0]], [.1, 0, self.origin[1]], [0, 0, 1]])
        self.indexing = 'xy'
        print(self.root)
    ...
    def get_worldcoord_from_worldgrid(self, worldgrid):
        ...
        coord_x = self.origin[0] + self.grid_cell * grid_x  # -90 + .1 * x
        coord_y = self.origin[1] + self.grid_cell * grid_y  # -50 + .1 * x
    ...
    def get_worldgrid_from_worldcoord(self, worldcoord):
        ...
        grid_x = (coord_x - self.origin[0]) / self.grid_cell  # (cx + 90) / .1 
        grid_y = (coord_y - self.origin[1]) / self.grid_cell  # (cy + 50) / .1 
        return np.array([grid_x, grid_y], dtype=int)
